...
function randomNumber1(min, max) {
    var number = Math.trunc(Math.random() * (max - min) + min);
    return number;
}
...
*/
function randNumDec1(min, max, precision) {
    var multFactor = Math.pow(10, precision); //il metodo pow() esegue la potenza: come primo argomento si passa la base, come secondo l'esponente a cui fa riferimento. 
